  ...
  try {
    ...
    if (!title || !promptText || uniqueCategories.length === 0) {
      return NextResponse.json(
        { error: "Please fill in all required fields (Title, Prompt, and at least one Category)." },
        { status: 400 }
      );
    }
    ...
    console.error("Add prompt API route error:", error);
    return NextResponse.json(
      { error: error.message || "An unexpected error occurred while saving the prompt." },
      { status: 500 }
    );
  }
}
